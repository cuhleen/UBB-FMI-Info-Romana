...
public:
	...
	Service(Repo& repository, Validator& validator, ShoppingCart& cosFilme) noexcept: repository{ repository }, validator{ validator }, cosFilme { cosFilme } {}
	...
	*/
	void adaugaFilm(const string& titluFilm, const string& genFilm, int anAparitie, const string& actorPrincipal) const ;
	...
	*/
	...
	arunca exceptie daca nu s-a gasit titlul filmului
	*/
	Film cautaFilm(const string& titluFilm) const;
	...
	*/
	...
	void adaugaInCos(const string& titluFilm);
	void stergeDinCos(const string& titluFilm);
	void elibereazaCos();
	void genereazaCos(int n);
	int getNumFilmeCos();
	vector<Film> getFilmeCos() const;
	void exportaCosInHTML(const string& numeFisier);
