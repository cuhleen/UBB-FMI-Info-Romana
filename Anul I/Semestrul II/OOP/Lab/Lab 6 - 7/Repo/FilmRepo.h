...
public:
    ...
    */
    virtual void modificaFilm(Film filmModificat, int pozitie);

    /*
    Cauta film in repository
    arunca exceptie daca nu se gaseste filmul
    */
    const Film& cautaFilm(const string& titlu);

    [[nodiscard]] int cautaPozitieDupaTitlu(const string& titluFilm) const;

    /*
    Functia getAll returneaza toate filmele din repository
    */
    [[nodiscard]] const VectorDinamic<Film>& getAll() const noexcept;
