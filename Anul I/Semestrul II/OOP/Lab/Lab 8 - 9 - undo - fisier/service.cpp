...
void Service::modificaFilm(const string& titluFilm, const string& genFilm, int anAparitie, const string& actorPrincipal) {
	const Film filmNou{titluFilm, genFilm, anAparitie, actorPrincipal};
	validator.valideaza(filmNou);
	const Film filmVechi = repository.cautaFilm(titluFilm);
	const int pozitie = repository.cautaPozitieDupaTitlu(titluFilm);
	repository.modificaFilm(filmNou, pozitie);
	undoActions.push_back(std::make_unique<UndoModifica>(repository, filmVechi));
}

void Service::undo() {
	if (undoActions.empty()) {
		throw ExceptieRepo("Nu mai exista operatii de undo!");
	}
...
void Service::incarcaDateDinFisier(const string& numeFisier) {
	repository.incarcaDinFisier(numeFisier);
}

void Service::salveazaDateInFisier(const string& numeFisier) {
	repository.salveazaInFisier(numeFisier);
}
