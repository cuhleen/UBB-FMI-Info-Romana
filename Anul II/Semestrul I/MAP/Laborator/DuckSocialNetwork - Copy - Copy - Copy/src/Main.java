...
import com.org.example.database.DatabaseInitializer;
import com.org.example.database.DatabaseManager;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        DatabaseManager.initializeDatabaseIfNeeded();
        try (Connection conn = DatabaseConnection.getConnection()) {
            System.out.println("Conectată la PostgreSQL!");
            DatabaseInitializer.createAllTables(conn);
        } catch (SQLException e) {
            System.err.println("Eroare la conexiune PostgreSQL: " + e.getMessage());
            return;
        }

        String dbUrl = "jdbc:postgresql://localhost:5432/duckdb";
        String dbUser = "duckuser";
        String dbPass = "parola123";
        ...
        RaceEventService raceEventService = new RaceEventService(repoEvent, repoDuck, personService);
        ...
        System.out.println("\n=== Aplicația este pornită și conectată la baza de date PostgreSQL! ===");
        mainMenu();
    }
    ...
    private static void mainMenu() {
        ...
        while (running) {
            ...
            System.out.println("5. Race Events");
            System.out.println("6. Community Analysis");
            System.out.println("7. Run Race Events");
            ...
            switch (choice) {
                ...
                case 2 -> personMenu();
                case 3 -> cardMenu();
                case 4 -> friendshipMenu();
                case 5 -> raceEventMenu();
                case 6 -> communityAnalysisMenu();
                case 7 -> runRaceEvents();
                case 8 -> {
                    running = false;
    ...
    private static void updateDuck() {
        ...
        if (duckOpt.isPresent()) {
            ...
            duck.setTip(TipRata.valueOf(type));
            duck.setViteza(speed);
            duck.setRezistenta(resistance);

            // Actualizează în baza de date
            boolean success = appService.getDuckService().updateDuck(duck);
            if (success) {
    ...
    private static void updatePerson() {
        ...
        if (personOpt.isPresent()) {
            ...
            System.out.print("Enter new username (current: " + person.getUsername() + "): ");
            String username = scanner.nextLine();
            System.out.print("Enter new email (current: " + person.getEmail() + "): ");
            String email = scanner.nextLine();
            System.out.print("Enter new password: ");
            String password = scanner.nextLine();
            System.out.print("Enter new first name (current: " + person.getPrenume() + "): ");
            String prenume = scanner.nextLine();
            System.out.print("Enter new last name (current: " + person.getNume() + "): ");
            String nume = scanner.nextLine();
            System.out.print("Enter new birth date (YYYY-MM-DD) (current: " + person.getDataNasterii() + "): ");
            String dataNasteriiStr = scanner.nextLine();
            System.out.print("Enter new occupation (current: " + person.getOcupatie() + "): ");
            String ocupatie = scanner.nextLine();
            System.out.print("Enter new empathy level (current: " + person.getNivelEmpatie() + "): ");
            double nivelEmpatie = scanner.nextDouble();
            scanner.nextLine(); // consume newline

            // Actualizează obiectul Person
            person.setUsername(username);
            person.setEmail(email);
            person.setPassword(password);
            person.setPrenume(prenume);
            person.setNume(nume);
            person.setDataNasterii(LocalDate.parse(dataNasteriiStr));
            person.setOcupatie(ocupatie);
            person.setNivelEmpatie(nivelEmpatie);

            // Salvează modificările în baza de date
            boolean success = appService.getPersonService().updatePerson(person);
            if (success) {
    ...
    private static void deletePerson() {
        ...
        System.out.println("Person deletion executed via AppService.");
    }
    ...
    private static void updateCard() {
        ...
        if (cardOpt.isPresent()) {
            ...
            card.setNumeCard(newName);

            // Salvează în baza de date
            boolean success = appService.getCardService().updateCard(card);

            if (success) {
                System.out.println("Card updated successfully!");
            } else {
                System.out.println("Failed to update card in database.");
            }
        } else {
            System.out.println("Card not found!");
        }
    ...
    private static void createRaceEvent() {
        ...
        Optional<Person> creator = appService.getPersonService().getPerson(creatorId);
