...
public class RaceEventService {
    ...
    private final RepoPersonDB repoPerson;

    // cache in-memory
    private final Map<Long, RaceEvent> events = new HashMap<>();

    public RaceEventService(RepoEventDB repoEvent, RepoDuckDB repoDuck, RepoPersonDB repoPerson) {
    ...
    public RaceEvent createRaceEvent(String title, String desc, long creatorId, List<Double> balize) {
        ...
        // Fetch the creator from either ducks or persons repository
        User creator = null;

        // First try to find in ducks
        Optional<Duck> duckOpt = repoDuck.findById(creatorId);
        if (duckOpt.isPresent()) {
            creator = duckOpt.get();
        } else {
            // If not found in ducks, try persons
            Optional<Person> personOpt = repoPerson.findById(creatorId);
            if (personOpt.isPresent()) {
                creator = personOpt.get();
            }
        }
