...
public class RepoDuckDB {
    ...
    public List<Duck> findAll() {
        ...
        String sql = """
        SELECT d.*, c.id AS card_id, c.nume_card AS card_name
        FROM ducks d
        ...
        try (Connection connection = DriverManager.getConnection(url, user, pass);
            ...
            while (rs.next()) {
                ...
                if (cardName != null) {
                    ...
                    core.model.Card card = new core.model.Card(cardId, cardName);
                    d.setCard(card);
                }
    ...
    // PAGINATION
    // ---------------------------------------
    public List<Duck> findPage(int limit, int offset) {
        List<Duck> ducks = new ArrayList<>();

        String sql = "SELECT * FROM ducks ORDER BY id LIMIT ? OFFSET ?";
        ...
        try (Connection connection = DriverManager.getConnection(url, user, pass);
            ...
            System.err.println("[EROARE RepoDuckDB.findPage] " + e.getMessage());
        }
    ...
    public int count() {
        ...
        try (Connection connection = DriverManager.getConnection(url, user, pass);
            ...
            System.err.println("[EROARE RepoDuckDB.count] " + e.getMessage());
        }
